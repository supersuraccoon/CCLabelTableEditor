...
- (id)initWithPlaceHolder:(NSString*)placeHolder content:(NSString *)content maxCount:(NSUInteger)maxCount block:(void(^)(NSString *text))block
{
    self = [super init];
    if (self) {
        _shouldAnimate = YES;
        _maxCount = maxCount;
        if (block) block_ = [block copy];
...
{
    if ([notification object] != self) return;
    if ([self.text length] > _maxCount) return;
    [self stopObservingNotifications];
    if ([self.delegate respondsToSelector:@selector(popupTextView:willDismissWithText:)]) {
        [self.delegate popupTextView:self willDismissWithText:[self.text stringByReplacingOccurrencesOfString:@"\n" withString:@""]];
    }
    ...
    [UIView animateWithDuration:ANIMATION_DURATION animations:^{
        ...
        if (finished) {
            if ([self.delegate respondsToSelector:@selector(popupTextView:didDismissWithText:)]) {
                [self.delegate popupTextView:self didDismissWithText:[self.text stringByReplacingOccurrencesOfString:@"\n" withString:@""]];
            }
            if(block_) block_([self.text stringByReplacingOccurrencesOfString:@"\n" withString:@""]);
